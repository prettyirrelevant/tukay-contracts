...
describe('Airdrop', function (){
    ...
    async function deployAirdropFixture() {
        ...
        return {airdrop, airdropAddress, owner, otherAccount};
    }
    ...
    async function generateMerkleProof() {
        ...
        return {tree, merkleRoot, leaves, whitelisted, notWhitelisted}
    }
    ...
    describe('Creations', function () {
        ...
        it('Should create an airdrop with a valid ERC20 token', async function () {
            const { airdrop, airdropAddress, owner } = await loadFixture(deployAirdropFixture);
    ...
    describe('Participations', function () {
        ...
        it('Should fail if address has already withdrawn airdrop amount', async function () {
            ...
            expect(airdropParticipationTx).to.emit(airdrop, 'AirdropDistributed').withArgs(1, whitelisted[2].address, ethers.parseEther("0.1") / BigInt(whitelisted.length));
            await expect(airdrop.connect(whitelisted[2]).withdrawAirdropShare(1, tree.getHexProof(padBuffer(whitelisted[2].address)))).to.revertedWith('This address already claimed the airdrop');
        });
    })
})
